# Load last so the public API retains the exact-union discovery contract while
# replacing the multi-condition estimator directly, without an intermediate GLM.
...
project_condition_grn_cells <- function(
    ...
    answer <- .condition_legacy_project_condition_grn_cells(
        object = object, fit = fit, targets = targets,
        significant_only = significant_only,
        return_edge_contributions = return_edge_contributions
    )
    if (inherits(answer, "PandoConditionProjection") &&
        identical(fit$model_schema, .condition_multitask_ridge_schema)) {
        answer$projection_origin <- fit$fit_engine
        answer$coefficient_scale <- "raw_tf_atac_interaction_units"
        answer$internal_fit_scale <- fit$internal_predictor_scale
    }
    answer
}
