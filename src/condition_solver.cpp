...
std::vector<SparseMatrix<double>> as_sparse_list(const Rcpp::List& input) {
    ...
    for (R_xlen_t i = 0; i < input.size(); ++i) {
        ...
        if (Rf_isS4(value) && Rf_inherits(value, "sparseMatrix")) {
            ...
            Eigen::Map<MatrixXd> mapped(dense.begin(), dense.nrow(), dense.ncol());
...
SmoothResult profiled_smooth(
    ...
    SmoothResult out{0.0, MatrixXd::Zero(p, tasks), VectorXd::Zero(tasks)};
    for (int task = 0; task < tasks; ++task) {
        VectorXd xb = X[task] * B.col(task);
        out.intercept[task] = (y[task] - xb).mean();
        VectorXd residual = y[task].array() - out.intercept[task] - xb.array();
        out.value += 0.5 * loss_weights[task] * residual.squaredNorm();
...
double sparse_group_penalty(
    ...
    return strength * ((1.0 - condition_mix) * group + condition_mix * element);
}
...
MatrixXd sparse_group_prox(
    ...
    const double element_threshold = step * lambda * alpha * condition_mix;
    const double group_threshold = step * lambda * alpha * (1.0 - condition_mix);
    for (int row = 0; row < V.rows(); ++row) {
        for (int task = 0; task < V.cols(); ++task) {
            if (!mask(row, task)) continue;
            const double value = V(row, task);
            const double magnitude = std::max(std::abs(value) - element_threshold, 0.0);
            U(row, task) = std::copysign(magnitude, value);
        }
        const double norm = U.row(row).norm();
        if (norm > 0.0) {
            const double scale = std::max(1.0 - group_threshold / norm, 0.0);
            U.row(row) *= scale;
        }
    }
    return U;
...
double centered_squared_norm(const SparseMatrix<double>& X) {
    ...
    VectorXd mean = (X.transpose() * ones) / static_cast<double>(X.rows());
    return std::max(X.squaredNorm() - X.rows() * mean.squaredNorm(), 0.0);
}
...
Rcpp::List fit_one_lambda(
    ...
    if (!std::isfinite(step) || step <= 0.0) {
        ...
        for (int task = 0; task < static_cast<int>(X.size()); ++task) {
            upper += loss_weights[task] * centered_squared_norm(X[task]);
        }
        step = (!std::isfinite(upper) || upper <= 0.0) ? 1.0 : 1.0 / upper;
    }
    SmoothResult initial = profiled_smooth(B, X, y, loss_weights, ridge);
    double objective_previous = initial.value +
    ...
    for (iteration = 1; iteration <= max_iter; ++iteration) {
        SmoothResult smooth_Z = profiled_smooth(Z, X, y, loss_weights, ridge);
        MatrixXd candidate;
        ...
        while (true) {
            ...
            smooth_candidate = profiled_smooth(candidate, X, y, loss_weights, ridge);
        ...
        double objective_candidate = smooth_candidate.value +
            sparse_group_penalty(candidate, lambda, alpha, condition_mix);
        if (objective_candidate > objective_previous + 1e-10) {
            acceleration = 1.0;
            Z = B;
            smooth_Z = profiled_smooth(Z, X, y, loss_weights, ridge);
            while (true) {
                candidate = sparse_group_prox(
                    Z - step * smooth_Z.gradient,
                    mask,
                ...
                step *= backtrack;
                if (step < min_step) {
                    Rcpp::stop("Backtracking line search reached min_step after restart.");
                }
            }
        ...
        objective_change = std::abs(objective_candidate - objective_previous) /
            (std::abs(objective_previous) + std::numeric_limits<double>::epsilon());
        ...
        const double acceleration_new =
            (1.0 + std::sqrt(1.0 + 4.0 * acceleration * acceleration)) / 2.0;
        MatrixXd Z_new = B + ((acceleration - 1.0) / acceleration_new) *
            (B - B_previous);
...
Rcpp::List condition_fit_multitask_path_cpp(
    ...
    if (X_list.size() < 2 || X_list.size() != y_list.size()) {
        Rcpp::stop("X_list and y_list must contain the same two or more conditions.");
    }
    if (!std::isfinite(alpha) || alpha < 0.0 || alpha > 1.0 ||
        !std::isfinite(condition_mix) || condition_mix < 0.0 || condition_mix > 1.0) {
        Rcpp::stop("alpha and condition_mix must be finite values in [0, 1].");
    }
    if (max_iter < 1 || !std::isfinite(tol_objective) || tol_objective <= 0.0 ||
    ...
    if (coefficient_mask.nrow() != p || coefficient_mask.ncol() != tasks) {
        Rcpp::stop("coefficient_mask dimensions do not match predictors and conditions.");
    }
    Eigen::ArrayXXi mask(p, tasks);
    for (int row = 0; row < p; ++row) {
        int eligible = 0;
        for (int task = 0; task < tasks; ++task) {
            if (Rcpp::LogicalVector::is_na(coefficient_mask(row, task))) {
                Rcpp::stop("coefficient_mask cannot contain NA.");
            }
            mask(row, task) = coefficient_mask(row, task) ? 1 : 0;
            eligible += mask(row, task);
        }
        if (!eligible) Rcpp::stop("Every predictor requires an eligible condition.");
    }
    VectorXd loss_weights(tasks);
    double total_n = 0.0;
    for (int task = 0; task < tasks; ++task) {
        if (X[task].cols() != p || X[task].rows() != y[task].size() ||
            X[task].rows() < 1) {
    ...
    for (R_xlen_t index = 0; index < lambda.size(); ++index) {
        ...
        Rcpp::List fit = fit_one_lambda(
            X, y, loss_weights, mask, current_lambda, alpha, condition_mix,
            B, step, max_iter, tol_objective, tol_coef, keep_history
        );
